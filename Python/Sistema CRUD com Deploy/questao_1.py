...
class ControleEstoque:  # Aqui é a lógica do sistema em si
    ...
    def __init__(self):
        ...
        self.set_codigos = (
            set()
        )  # Cria um set vazio, ja que 2 produtos não possuem o mesmo código
        self.tupla_categorias: tuple[str, str, str, str] = (
            "Alimento",
            "Limpeza",
            "Higiene",
            "Outros",
        )  # Apenas as 4 categorias, nada de muito especial.
        self.lista_movimentacoes = []
        self.carregar_dados()  # aaaaaa
        ####a

    def carregar_dados(self) -> None | bool:  # Usa JSON
    ...
    def buscar_nome(self, nome: str) -> list | Exception:
        encontrados = [
            ...
            if nome.lower() in str(p["Nome"]).lower()
        ]
        if encontrados:
            return encontrados
        else:
    ...
    def listar_por_categoria(self, categoria: str) -> list:
        lista_categoria_temp = [
            p
            for p in self.lista_dict_produtos
            if str(p["Categoria"]).lower() == categoria.lower()
        ]
    ...
    def cadastrar_produto(
        self, nome: str, codigo: int, categoria: str, preco: float, quantidade: float
    ) -> None:  # Cria e add um produto novo
        if codigo in self.set_codigos:
            raise ValueError("Código já existente!")
        if categoria not in self.tupla_categorias:
            raise ValueError(
                f"Categoria inexistente! Categorias possíveis: {self.tupla_categorias}"
            )
        if len(nome) < 1:
    ...
    def atualizar_produto(self, codigo: int, campo: str, novo_valor: str | int | float):
        if codigo not in self.set_codigos or campo.lower() not in [
            "nome",
            "categoria",
            "preço",
            "estoque",
    ...
    def movimentar_produto(
        self, data: str, produto: str, tipo: str, quantidade: float
    ) -> None:
        if self.buscar_nome(produto):
            self.lista_movimentacoes.append(
                {"data": data, "produto": produto, "tipo": tipo, "qnt": quantidade}
            )
...
class FrontEnd:
        ...
        with st.form("form_cadastro"):
            ...
            with col4:
                quantidade = st.number_input(
                    "Quantidade em estoque", min_value=0.0, format="%.2f"
                )
            ...
            if clique_salvar:
                try:
                    ...
                    st.rerun()
                    st.success(
                        f"Produto {codigo} adicionado com sucesso (Código: {codigo})"
                    )
                except Exception as e:
                    st.error(
                        f"Falha no cadastro! Verifique se o código já existe. Código de erro: {e}"
                    )
        return
        ...
        with st.form("form_atualizar"):
            ...
            if tipo_de_dado in ["Preço", "Estoque"]:
                novo_valor = st.number_input(
                    "Qual seria o novo valor?", min_value=0, key="upd_valor_num"
                )
            elif tipo_de_dado == "Código":
                novo_valor = st.number_input(
                    "Qual seria o novo valor?", min_value=1, key="upd_valor_cod"
                )
            ...
            if clique_salvar:
                ...
                try:
                    ...
                    st.success(
                        f"Produto {codigo} atualizado com sucesso! Novo {tipo_de_dado}: {novo_valor}"
                    )
                    st.rerun()
                except Exception as e:
    ...
    def renderizar_buscar(self) -> None:
        ...
        tipo_busca = st.selectbox(
            "Tipo de busca", ("Nome", "Categoria", "Estoque Baixo")
        )
        if tipo_busca == "Nome":
    ...
    def renderizar_excluir(self) -> None:
        ...
        with st.form("form_exclusao_busca"):
            codigo_excluir_input = st.number_input(
                "Insira o código (ID) do produto a ser excluído:",
                min_value=1,
                step=1,
        ...
        if codigo_excluir and codigo_excluir in self.estoque.set_codigos:
            nome_produto = next(
                (
                    p["Nome"]
                    for p in self.estoque.lista_dict_produtos
                    if int(p["Código"]) == int(codigo_excluir)
                ),
                "Produto Desconhecido",
            )
            st.error(
                f"Você está prestes a excluir o produto: {nome_produto} (Código: {int(codigo_excluir)})"
            )
            ...
            codigo_excluir is not None
            and codigo_excluir not in self.estoque.set_codigos
        ):
    ...
    def renderizar_home(self):
        ...
        try:
            st.dataframe(
                self.estoque.lista_dict_produtos, width="stretch", hide_index=True
            )
        except Exception as e:
            print(f"Erro! {e}")
